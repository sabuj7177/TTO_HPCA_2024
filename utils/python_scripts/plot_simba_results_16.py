...
def draw_graph(ax, folder_path, names, total_nodes, schemes, folder_names, scalesim_config):
    benchmarks = ['alexnet', 'AlphaGoZero', 'FasterRCNN', 'Googlenet', 'NCF_recommendation', 'Resnet152', 'Transformer']
    training_time = [343080190, 4736220, 56703838, 26788507, 2390027, 146240671, 21537734]
    ...
    for s, name in enumerate(names):
        for b, bench in enumerate(benchmarks):
            if name == 'multitree' or name == 'ring_bi' or name == 'ring' or name == 'ring_odd' or name == 'ring_odd_bi' or name == 'dtree' or name == 'ring2dn':
                filename = "%s/%s/%s_%s_%d_mesh_200_%s_%s.json" % (folder_path, folder_names[s], bench, name, total_nodes, bench, scalesim_config)
            else:
                filename = "%s/%s/%s_%s_%d_mesh_%s_%s.json" % (folder_path, folder_names[s], bench, name, total_nodes, bench, scalesim_config)
...
def main():
    ...
    folder_path = '{}/HPCA_2024_final/simba/comm'.format(os.environ['SIMHOME'])
    ...
    legend = draw_graph(ax1, folder_path, even_names, 36, schemes_evens, folder_names, 'express_16')
    ...
    figure.savefig('models_simba_16.pdf', bbox_inches='tight')
