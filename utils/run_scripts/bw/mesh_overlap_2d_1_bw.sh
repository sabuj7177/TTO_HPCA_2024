...
outdir=$SIMHOME/HPCA2024/bandwidth/mesh_overlap_2d_1
mkdir -p $outdir
...
for i in ${!syntheticDataSize[@]}; do
  python $SIMHOME/src/simulate.py \
      ...
      --strict-schedule \
      --prioritize-schedule \
      > $outdir/bw_${syntheticDataSize[$i]}_mesh_overlap_2d_1_16_mesh_error.log 2>&1 &
  ...
  python $SIMHOME/src/simulate.py \
      ...
      --strict-schedule \
      --prioritize-schedule \
      > $outdir/bw_${syntheticDataSize[$i]}_mesh_overlap_2d_1_25_mesh_error.log 2>&1 &
  ...
  python $SIMHOME/src/simulate.py \
      ...
      --strict-schedule \
      --prioritize-schedule \
      > $outdir/bw_${syntheticDataSize[$i]}_mesh_overlap_2d_1_64_mesh_error.log 2>&1 &
