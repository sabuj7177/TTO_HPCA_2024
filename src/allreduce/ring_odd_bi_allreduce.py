...
from find_odd_ring import find_ring

class RingOddBiAllreduce(Allreduce):
    ...
    def compute_trees(self, kary=None, alternate=True, sort=False, verbose=True):
        ...
        self.ring = find_ring(numpy.sqrt(self.network.nodes))

        if len(self.ring) == self.network.nodes-1:
            self.timesteps = self.network.nodes - 2
            self.num_flows = self.network.nodes*4
            if verbose:
            ...
            for root in range(self.network.nodes):
                ...
                self.trees[root*4+2] = []
                self.trees[root*4+3] = []
                if root == self.ring[0]+1:
                    # center node
                    for t in range(int((self.network.nodes-1))):
                        self.trees[root*4].append((root, root-1, t))
                        #self.trees[root*4+1].append((root, root-int(numpy.sqrt(self.network.nodes)), t))
                        self.trees[root*4+2].append((root, root+1, t))
                        #self.trees[root*4+3].append((root, root+int(numpy.sqrt(self.network.nodes)), t))
                    print('ring {}: {}'.format(root*4, self.trees[root*4]))
                    print('ring {}: {}'.format(root*4+1, self.trees[root*4+1]))
                    print('ring {}: {}'.format(root*4+2, self.trees[root*4+2]))
                    print('ring {}: {}'.format(root*4+3, self.trees[root*4+3]))
                    continue
                # elif root == self.ring[0]:
                #     for t in range(int((self.network.nodes-1)/2)):
                #         self.trees[root*4+1].append((root+1, root, t))
                # elif root == self.ring[0]+2:
                #     for t in range(int((self.network.nodes-1)/2)):
                #         self.trees[root*4+1].append((root-1, root, t))
                # elif root == self.ring[0]+1-int(numpy.sqrt(self.network.nodes)):
                #     for t in range(int((self.network.nodes-1)/2)):
                #         self.trees[root*4+1].append((root+int(numpy.sqrt(self.network.nodes)), root, t))
                # elif root == self.ring[0]+1+int(numpy.sqrt(self.network.nodes)):
                #     for t in range(int((self.network.nodes-1)/2)):
                #         self.trees[root*4+1].append((root-int(numpy.sqrt(self.network.nodes)), root, t))

                root_idx = self.ring.index(root)
                for timestep in range(self.network.nodes - 2):
                    parent_idx = (timestep + root_idx) % (self.network.nodes-1)
                    child_idx = (timestep + root_idx + 1) % (self.network.nodes-1)
                    parent_idx_reverse = (root_idx - timestep) % (self.network.nodes-1)
                    child_idx_reverse = (root_idx - timestep - 1) % (self.network.nodes-1)
                    ...
                    self.trees[root*4].append((child, parent, timestep))
                    self.trees[root*4+1].append((child_reverse, parent_reverse, timestep))
                if verbose:
                    print('ring {}: {}'.format(root*4, self.trees[root*4]))
                    print('ring {}: {}'.format(root*4+1, self.trees[root*4+1]))
                    print('ring {}: {}'.format(root*4+2, self.trees[root*4+2]))
    ...
    def generate_schedule(self, verbose=False):
        ...
        self.trees_parent = {}
        self.trees_children = {}
        for root in range(self.network.nodes*4):
        ...
        for node in range(self.network.nodes):
            ...
            datasize = 0.5*(1/(self.network.nodes-1))*(self.network.nodes)

            if node == self.ring[0]+1:
                # center
                for t in range(int((self.network.nodes-1))):
                    self.reduce_scatter_schedule[node].append({node*4: ((node-1, 2), [], datasize, t), node*4+2: ((node+1, 2), [], datasize, t)})
                # for t in range(1, int((self.network.nodes-1)/2) + 1, 1):
                #     # self.all_gather_schedule[node].append({(self.ring[0]+1)*4: ([], ((self.ring[0]+1)*4, node-1), datasize, self.network.nodes + t + 1), (self.ring[0]+1)*4+1: ([], ((self.ring[0]+1)*4+1, node-int(numpy.sqrt(self.network.nodes))), datasize, self.network.nodes + t + 1), (self.ring[0]+1)*4+2: ([], ((self.ring[0]+1)*4+2, node+1), datasize, self.network.nodes + t + 1), (self.ring[0]+1)*4+3: ([], ((self.ring[0]+1)*4+3, node+int(numpy.sqrt(self.network.nodes))), datasize, self.network.nodes + t + 1)})
                #     self.all_gather_schedule[node].append({})
                if verbose:
            ...
            child = self.ring[(index + 1) % (self.network.nodes-1)]

            neighbors = [self.ring[0], self.ring[0]+2]

            # reduce-scatter scheduled from 'leaf'
            rs_subflow = child*4
            rs_subflow_reverse = parent*4+1
            if node == neighbors[0]:
                self.reduce_scatter_schedule[node].append({rs_subflow: ((parent, 0), [((self.ring[0]+1)*4, self.ring[0]+1)], datasize, 0), rs_subflow_reverse: ((child, 1), [], datasize, 0)})
            elif node == neighbors[1]:
                self.reduce_scatter_schedule[node].append({rs_subflow: ((parent, 0), [], datasize, 0), rs_subflow_reverse: ((child, 1), [((self.ring[0]+1)*4+2, self.ring[0]+1)], datasize, 0)})
            else:
            ...
            ag_subflow = node*4
            ag_subflow_reverse = node*4+1
            if node in neighbors:
                self.all_gather_schedule[node].append({ag_subflow: ([(child, 0)], None, datasize, self.network.nodes), ag_subflow_reverse: ([(parent, 1)], None, datasize, self.network.nodes), (self.ring[0]+1)*4+neighbors.index(node)*2: ([(self.ring[0]+1, neighbors.index(node))], None, datasize, self.network.nodes)})
            else:
            ...
            for i in range(self.network.nodes - 3):
                # reduce-scatter
                rs_subflow = self.ring[(index + i + 2) % (self.network.nodes-1)]*4
                rs_subflow_reverse = self.ring[(index - i - 2)]*4 + 1
                if node == neighbors[0]:
                    self.reduce_scatter_schedule[node].append({rs_subflow: ((parent, 0), [(rs_subflow, child), ((self.ring[0]+1)*4, self.ring[0]+1)], datasize, i + 1), rs_subflow_reverse: ((child, 1), [(rs_subflow_reverse, parent)], datasize, i+1)})
                elif node == neighbors[1]:
                    self.reduce_scatter_schedule[node].append({rs_subflow: ((parent, 0), [(rs_subflow, child)], datasize, i + 1), rs_subflow_reverse: ((child, 1), [(rs_subflow_reverse, parent), ((self.ring[0]+1)*4+2, self.ring[0]+1)], datasize, i+1)})
                else:
                ...
                # all-gather
                ag_subflow = self.ring[index - i - 1]*4
                ag_subflow_reverse = self.ring[(index + i + 1) % (self.network.nodes-1)]*4 + 1
                
                if node == neighbors[0]:
                    self.all_gather_schedule[node].append({ag_subflow: ([(child, 0)], (ag_subflow, parent), datasize, i + 1 + self.network.nodes), ag_subflow_reverse: ([(parent, 1)], (ag_subflow_reverse, child), datasize, i + 1 + self.network.nodes), (self.ring[0]+1)*4: ([(self.ring[0]+1, 0)], (ag_subflow, parent), datasize, self.network.nodes + i + 1)})
                elif node == neighbors[1]:
                    self.all_gather_schedule[node].append({ag_subflow: ([(child, 0)], (ag_subflow, parent), datasize, i + 1 + self.network.nodes), ag_subflow_reverse: ([(parent, 1)], (ag_subflow_reverse, child), datasize, i + 1 + self.network.nodes), (self.ring[0]+1)*4+2: ([(self.ring[0]+1, 1)], (ag_subflow_reverse, child), datasize, self.network.nodes + i + 1)})
                else:
            ...
            self.reduce_scatter_schedule[node].append({node*4: ((None, None), [(node*4, child)], 0, self.network.nodes - 1), node*4+1: ((None, None), [(node*4+1, parent)], 0, self.network.nodes - 1)})
