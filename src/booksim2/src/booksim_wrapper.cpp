...
tuple<int, int, Message::MessageType, bool, int> BookSim::PeekMessage(int node, int vnet)
{
  int flow = -1;
  int src = -1;
  ...
  return make_tuple(flow, src, type, end, timestep);
}
  ...
  while (!_traffic_manager->Idle() || _outstanding_messages > 0) {
    ...
    for (int node = 0; node < gNodes; node++) {
      for (int vnet = 0; vnet < 2; vnet++) {
        tuple<int, int, Message::MessageType, bool, int> result = PeekMessage(node, vnet);
