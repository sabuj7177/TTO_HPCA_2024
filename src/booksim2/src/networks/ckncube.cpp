...
void CKNCube::_BuildNet(const Configuration &config)
{
  ...
  vector<int> link_latencies;
  vector<int> link_bandwidths = config.GetIntArray("link_bandwidths");
  double channel_width = (double)config.GetInt("channel_width");
  for(size_t i=0; i<link_bandwidths.size(); i++){
    link_latencies.push_back(std::ceil((channel_width*1.0)/link_bandwidths[i]));
  }
...
//        latency = 150 * 2;
//      }
      int latency = link_latencies[dim]; // TODO: parameterize
...
//      cout << "Router " << router << " Dim " << dim << " " << left_input << " " << right_input << " " << left_output << " " << right_output << endl;
